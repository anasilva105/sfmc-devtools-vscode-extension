...
async function noPrerequisitesHandler(context: ExtensionContext) {
  let response = await window.showInformationMessage(
    "You don't have the prerequisites necessary to run sfmc-devtool." +
  ...
  if (response.toLocaleLowerCase() === 'yes') {
    ...
    const stylePathOnDisk = Uri.file(
      path.join(
        ...
        'dist/design-system/styles/salesforce-lightning-design-system.css'
      )
    );
    const styleUri = panel.webview.asWebviewUri(stylePathOnDisk);
    html = html.replace('{{styleUri}}', styleUri.toString());

    const jsPathOnDisk = Uri.file(
      path.join(context.extensionPath, 'src', 'js', 'mcdev_prerequisites.js')
    );
...
async function noDevToolsHandler(context: ExtensionContext) {
  let response = await window.showInformationMessage(
    'Cannot find sfmc-devtools.' + ' Do you want to install it?',
    ...['Yes', 'No']
  );
