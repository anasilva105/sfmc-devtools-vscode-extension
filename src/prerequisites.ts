...
const PREREQUISITES: any = {
  ...
  git: {
    ...
    isValidVersion: (version: string) =>
      version.match(/git version \d*.\d*.\d*.*/)
  }
};
export async function hasPrerequisitesHandler(command: any): Promise<boolean> {
  let prerequisitesList: any = [];
  if (!command) {
    prerequisitesList = await Promise.all(
      Object.keys(PREREQUISITES).map(async (res) => {
        let result = await window.createTerminal(
          PREREQUISITES[res].cmd_version
        );
    ...
    let result = await window.createTerminal(
      PREREQUISITES[command].cmd_version
    );
